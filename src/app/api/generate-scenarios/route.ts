...
// ── Demo fallback nodes (when GEMINI_API_KEY is missing) ──
function getDemoNodes(nodeCount: number): any[] {
  const demoNodes = [
    {
      id: "concept-extraction",
      title: "Concept Extraction",
      crisis_scenario: "You're drowning in notes from a lecture. How do you identify the core concepts without missing critical details?",
      domino_question: "Walk me through the cascade: How does identifying key concepts unlock understanding of everything else?",
      formal_mechanism: "Breaking complex content into distinct concepts enables hierarchical learning and spaced repetition.",
      latex_formula: "\\text{Content} \\rightarrow \\text{Extraction} \\rightarrow \\text{Mastery}",
      so_what: "One solid concept beats ten fuzzy ideas.",
      stress_test: "What if the source material is contradictory or ambiguous?",
      multiple_choice_question: "What's the first step in learning any new topic?",
      multiple_choice_options: [
        { id: "A", text: "Memorize all the facts", is_correct: false },
        { id: "B", text: "Identify and extract core concepts", is_correct: true },
        { id: "C", text: "Solve practice problems", is_correct: false }
      ]
    }
  ];
  
  return demoNodes.slice(0, nodeCount);
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!hasGeminiKey) {
      const targetNodeCount = 5;
      return NextResponse.json({
        success: true,
        source_title: title,
        phase: "phase-0-extraction",
        logic_nodes: normaliseNodes(getDemoNodes(targetNodeCount)),
        nodeCount: targetNodeCount,
        _demo: true,
        _message: "Using demo nodes (GEMINI_API_KEY not configured)"
      });
    }
